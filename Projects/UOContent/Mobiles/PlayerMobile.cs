    ...
    public partial class PlayerMobile : Mobile, IHonorTarget, IHasSteps
    {
        ...
        */
        ...
        public override int GetMaxResistance(ResistanceType type)
        {
            ...
            if (type != ResistanceType.Physical && GlobalStatController.UnderCurseEffect(this))
            {
                max -= 10;
            }
        ...
        {
            // Call base implementation (Mobile.OnWarmodeChanged) which handles bandage interruption
            base.OnWarmodeChanged();

            // Sphere51a: Original PlayerMobile logic - ammo recovery
            if (!Warmode)
            {
        ...
        {
            // DUEL PIT SYSTEM - Check if both players are in the same duel pit session
            // Prevents criminal flagging during agreed-upon duels
            if (target is PlayerMobile &&
                Server.Engines.ConPVP.DuelPitController.IsInDuelSession(this) &&
                Server.Engines.ConPVP.DuelPitController.IsInDuelSession(target))
            {
                var mySession = Server.Engines.ConPVP.DuelPitController.GetActiveSession(this);
                var targetSession = Server.Engines.ConPVP.DuelPitController.GetActiveSession(target);

                // If both are in the same session, don't flag as criminal
                if (mySession != null && mySession == targetSession)
                {
                    return false;
                }
            }
        ...
        public bool HasBuff(BuffIcon b)
        {
            return m_BuffTable?.ContainsKey(b) == true;
        }
        ...
        // =====================================================
        // SPHERE51a: COMBAT SYSTEM HOOKS
        // =====================================================
        // These overrides implement player-specific combat mechanics for the 51alpha server.
        // See Master_Architecture.md Section 3.2-3.6 for complete behavior specifications.

        /// <summary>
        /// Sphere51a: Prevents weapon swinging during bandaging or spell casting.
        /// </summary>
        protected override bool ShouldPreventCombatSwing()
        {
            // Check if currently bandaging
            var bc = Server.Items.BandageContext.GetContext(this);
            if (bc != null)
            {
                return true; // Don't swing weapon while bandaging
            }

            // Sphere51a: Only prevent swinging when spell has ACTUALLY STARTED casting (delay timer running)
            // NOT when just selecting a spell or holding a target cursor
            // The delay timer starts when target is selected, which is when weapons get disarmed
            if (Spell is Spells.Spell spell && spell.HasDelayTimerRunning)
            {
                return true; // Don't swing weapon while spell delay timer is running
            }
        ...
        /// <summary>
        /// Sphere51a: Tracks outgoing damage dealt to other players for PvP context detection.
        /// </summary>
        protected override void OnDamageDealtToPlayer(Mobile victim, int amount)
        {
            if (victim == null || !victim.Player || victim == this)
                return;

            // Record outgoing damage for PvP context tracking
            Sphere51a.Core.S51aDamageTracker.RecordPlayerDamage(this, victim, amount);
        }

        /// <summary>
        /// Sphere51a: Interrupts bandaging when war mode is toggled.
        /// </summary>
        protected override void OnWarmodeToggled()
        {
            var bc = Server.Items.BandageContext.GetContext(this);
            ...
            {
                bc.StopHeal();
                SendLocalizedMessage(500969); // You stop applying bandages
            }
        }

        /// <summary>
        /// Sphere51a: Prevents weapon swing timer from starting when re-equipping during bandage.
        /// </summary>
        protected override void OnItemEquippedDuringAction(Item item)
        {
            var bc = Server.Items.BandageContext.GetContext(this);

            Utility.PushColor(ConsoleColor.Yellow);
            Console.WriteLine($"[BANDAGE DEBUG] OnItemEquippedDuringAction - Player: {Name}, Item: {item?.GetType().Name}, BandageContext: {bc != null}, NextCombatTime before: {NextCombatTime}");
            Utility.PopColor();

            if (bc != null && item is Server.Items.BaseWeapon)
            {
                Utility.PushColor(ConsoleColor.Magenta);
                Console.WriteLine($"[BANDAGE DEBUG] Item is weapon - OneHandedWeapon: {bc.OneHandedWeapon?.Serial}, TwoHandedWeapon: {bc.TwoHandedWeapon?.Serial}, Item: {item.Serial}");
                Utility.PopColor();

                // Check if this was one of the disarmed weapons
                if (item == bc.OneHandedWeapon || item == bc.TwoHandedWeapon)
                {
                    Utility.PushColor(ConsoleColor.Green);
                    Console.WriteLine($"[BANDAGE DEBUG] Re-freezing swing timer - setting NextCombatTime to {long.MaxValue}");
                    Utility.PopColor();
                    // Don't start swing timer until bandage completes
                    NextCombatTime = long.MaxValue;
                }
                else
                {
                    Utility.PushColor(ConsoleColor.Red);
                    Console.WriteLine($"[BANDAGE DEBUG] Weapon does NOT match stored weapons - timer NOT frozen!");
                    Utility.PopColor();
                }
            }
        }
