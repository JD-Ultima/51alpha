...
public class BandageContext : Timer
{
    ...
    // Sphere51a: Track disarmed weapons for swing timer reset
    public Item OneHandedWeapon { get; set; }
    public Item TwoHandedWeapon { get; set; }
    ...
    public void StopHeal()
    {
        ...
        // Sphere51a: Reset swing timer when bandage interrupted (players only)
        if (Healer.Player && Healer.NextCombatTime == long.MaxValue)
        {
            // Start fresh swing timer with weapon delay
            var weapon = Healer.Weapon;
            if (weapon is Items.BaseWeapon baseWeapon)
            {
                var delay = (int)baseWeapon.GetDelay(Healer).TotalMilliseconds;
                Healer.NextCombatTime = Core.TickCount + delay;
                Utility.PushColor(ConsoleColor.Magenta);
                Console.WriteLine($"[BANDAGE DEBUG] Bandage interrupted - starting fresh swing timer with {delay}ms delay");
                Utility.PopColor();
            }
            else
            {
                // Wrestling or no weapon - use default delay
                Healer.NextCombatTime = Core.TickCount + 2000; // 2 second delay for wrestling
                Utility.PushColor(ConsoleColor.Yellow);
                Console.WriteLine($"[BANDAGE DEBUG] Bandage interrupted - no weapon, using 2000ms wrestling delay");
                Utility.PopColor();
            }
        }
    ...
    protected override void OnTick()
    {
        ...
        // Sphere51a: Reset swing timer when bandage completes (players only)
        if (Healer.Player && Healer.NextCombatTime == long.MaxValue)
        {
            // Start fresh swing timer with weapon delay
            var weapon = Healer.Weapon;
            if (weapon is Items.BaseWeapon baseWeapon)
            {
                ...
                Console.WriteLine($"[BANDAGE DEBUG] Bandage complete - starting fresh swing timer with {delay}ms delay");
                Utility.PopColor();
            }
            else
            {
                ...
                Healer.NextCombatTime = Core.TickCount + 2000; // 2 second delay for wrestling
                Utility.PushColor(ConsoleColor.Yellow);
                Console.WriteLine($"[BANDAGE DEBUG] Bandage complete - no weapon, using 2000ms wrestling delay");
                Utility.PopColor();
            }
        }
    ...
    public static BandageContext BeginHeal(Mobile healer, Mobile patient)
    {
        ...
        {
            // Sphere51a: Interrupt any active spell casting when using bandages
            if (healer.Spell is Spells.Spell spell && spell.IsCasting)
            {
                spell.Disturb(Spells.DisturbType.UseItem);
            }

            // Sphere51a: Disarm weapons when bandaging (both hands)
            // Only affects players - NPCs/monsters not affected
            Item oneHandedWeapon = null;
            Item twoHandedWeapon = null;

            if (healer.Player)
            {
                var oneHanded = healer.FindItemOnLayer(Layer.OneHanded);
                var twoHanded = healer.FindItemOnLayer(Layer.TwoHanded);

                if (oneHanded != null && healer.Backpack != null)
                {
                    oneHandedWeapon = oneHanded;
                    healer.Backpack.DropItem(oneHanded);
                }

                if (twoHanded != null && healer.Backpack != null)
                {
                    twoHandedWeapon = twoHanded;
                    healer.Backpack.DropItem(twoHanded);
                }

                if (oneHandedWeapon != null || twoHandedWeapon != null)
                {
                    healer.SendLocalizedMessage(1062001); // You put your weapon away
                }
            }
            ...
            // Sphere51a: Store disarmed weapons for swing timer reset
            context.OneHandedWeapon = oneHandedWeapon;
            context.TwoHandedWeapon = twoHandedWeapon;

            // Sphere51a: Freeze weapon swing timer during bandaging (players only)
            if (healer.Player)
            {
                Utility.PushColor(ConsoleColor.Cyan);
                Console.WriteLine($"[BANDAGE DEBUG] Freezing swing timer - Player: {healer.Name}, NextCombatTime before: {healer.NextCombatTime}, setting to: {long.MaxValue}");
                Utility.PopColor();
                healer.NextCombatTime = long.MaxValue;
            }
