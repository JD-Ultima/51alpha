    ...
    : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability, IAosItem, IIdentifiable
{
    ...
    {
        // Check if Sphere51a equipment swap should apply (item in backpack or on ground within 2 tiles, NOT equipped)
        ...
        {
            HandleSphere51aEquip(from);
        }
        else
        {
            // When equipped or Sphere51a disabled, use default behavior
            base.OnDoubleClick(from);
        }
    }

    private void HandleSphere51aEquip(Mobile from)
    {
        // Check if item is in backpack or on ground within reach (2 tiles)
        bool canAccess = IsChildOf(from.Backpack) ||
                         (Parent == null && from.InRange(GetWorldLocation(), 2));

        if (!canAccess)
        {
            from.SendLocalizedMessage(500295); // "You are too far away"
            return;
        }
        ...
        if (Layer == Layer.TwoHanded)
        {
            // Remove shield (also on Layer.TwoHanded)
            Item shield = from.FindItemOnLayer(Layer.TwoHanded);
            if (shield != null && shield != this)
            {
                if (from.Backpack != null && from.Backpack.CheckHold(from, shield, true))
                {
                    from.Backpack.AddItem(shield);
                }
                else
                {
                    shield.MoveToWorld(from.Location, from.Map);
                    from.SendMessage("Your shield has been dropped at your feet.");
                }
            }

            // Remove single-handed weapon (on Layer.OneHanded)
            Item oneHanded = from.FindItemOnLayer(Layer.OneHanded);
            if (oneHanded != null)
            {
                if (from.Backpack != null && from.Backpack.CheckHold(from, oneHanded, true))
                {
                    from.Backpack.AddItem(oneHanded);
                }
                else
                {
                    oneHanded.MoveToWorld(from.Location, from.Map);
                    from.SendMessage("You are overweight. The item has been dropped at your feet.");
                }
            }
        }
        // CASE 2: Equipping a ONE-HANDED weapon
        else if (Layer == Layer.OneHanded)
        {
            // Remove two-handed weapon (occupies Layer.TwoHanded)
            Item twoHanded = from.FindItemOnLayer(Layer.TwoHanded);
            if (twoHanded != null && twoHanded is BaseWeapon)
            {
                if (from.Backpack != null && from.Backpack.CheckHold(from, twoHanded, true))
                {
                    from.Backpack.AddItem(twoHanded);
                }
                else
                {
                    twoHanded.MoveToWorld(from.Location, from.Map);
                    from.SendMessage("You are overweight. The item has been dropped at your feet.");
                }
            }

            // Remove current one-handed weapon
            Item currentEquipped = from.FindItemOnLayer(Layer.OneHanded);
            if (currentEquipped != null && currentEquipped != this)
            {
                if (from.Backpack != null && from.Backpack.CheckHold(from, currentEquipped, true))
                {
                    from.Backpack.AddItem(currentEquipped);
                }
                else
                {
                    currentEquipped.MoveToWorld(from.Location, from.Map);
                    from.SendMessage("You are overweight. The item has been dropped at your feet.");
                }
            }
        }
