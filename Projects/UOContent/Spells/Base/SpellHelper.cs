...
namespace Server.Spells
{
    ...
    public static class SpellHelper
    {
        ...
        {
            // Player-involved spells always use static magnitude of 10
            if (IsPlayerInvolved(caster, target))
            {
                return 10;
            }

            // Monster vs Monster: use existing skill-based calculation
            if (!Core.AOS)
            {
        ...
        public static bool IsPlayerInvolved(Mobile caster, Mobile target)
        {
            return caster is PlayerMobile || target is PlayerMobile;
        }
