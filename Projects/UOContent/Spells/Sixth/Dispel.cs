...
namespace Server.Spells.Sixth
{
    public class DispelSpell : MagerySpell, ITargetingSpell<Mobile>
    {
        ...
        {
            // Handle PlayerMobile - remove all stat effects
            if (m is PlayerMobile)
            {
                if (CheckHSequence(m, () =>
                {
                    ...
                    GlobalStatController.ClearAllStatEffects(m);
            ...
            // Handle dispellable creatures (original behavior)
            else if (m is BaseCreature { IsDispellable: true } bc)
            {
                if (CheckHSequence(m, () =>
                {
                    SpellHelper.Turn(Caster, m);
