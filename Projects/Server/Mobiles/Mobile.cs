...
public partial class Mobile : IHued, IComparable<Mobile>, ISpawnable, IObjectPropertyListEntity, IValueLinkListNode<Mobile>
{
    ...
    {
        var timerDiff = Core.TickCount - NextCombatTime;

        if (timerDiff < 0)
        {
            // Only log if timer is frozen (very close to expiring from long.MaxValue)
            if (Player && NextCombatTime == long.MaxValue)
            {
                // Timer is frozen - only log occasionally to reduce spam
                if (Core.TickCount % 1000 < 50) // Log roughly once per second
                {
                    Utility.PushColor(ConsoleColor.Green);
                    Console.WriteLine($"[COMBAT DEBUG] Timer FROZEN (NextCombatTime = long.MaxValue)");
                    Utility.PopColor();
                }
            }
            return;
        ...
        // Timer expired - log for players
        if (Player)
        {
            Utility.PushColor(ConsoleColor.Yellow);
            Console.WriteLine($"[COMBAT DEBUG] Timer EXPIRED - Player: {Name}, NextCombatTime: {NextCombatTime}, TickCount: {Core.TickCount}, checking ShouldPreventCombatSwing()");
            Utility.PopColor();
        }

        // Sphere51a: Hook for player-specific combat restrictions (bandaging, etc.)
        if (Player && ShouldPreventCombatSwing())
        {
            Utility.PushColor(ConsoleColor.Magenta);
            Console.WriteLine($"[COMBAT DEBUG] ShouldPreventCombatSwing returned TRUE - preventing swing");
            Utility.PopColor();
            return;
        }
        ...
        if (Player)
        {
            ...
            Console.WriteLine($"[COMBAT DEBUG] SWING ALLOWED - proceeding with attack!");
            Utility.PopColor();
        }
    ...
    /// <summary>
    ///     Sphere51a: Hook for player-specific combat restrictions.
    ///     Returns true if weapon swinging should be prevented (e.g., during bandaging or spell casting).
    ///     Override in PlayerMobile to implement bandage/spell swing restrictions.
    /// </summary>
    /// <returns>True to prevent weapon swing, false to allow</returns>
    protected virtual bool ShouldPreventCombatSwing()
    {
        // Base implementation: only prevent swinging during spell casting
        return Spell != null && Spell.IsCasting;
    }

    /// <summary>
    ///     Sphere51a: Hook for tracking outgoing damage dealt to other players.
    ///     Override in PlayerMobile to implement PvP context detection.
    /// </summary>
    /// <param name="victim">The mobile that was damaged</param>
    /// <param name="amount">Amount of damage dealt</param>
    protected virtual void OnDamageDealtToPlayer(Mobile victim, int amount)
    {
    }

    /// <summary>
    ///     Sphere51a: Hook for war mode toggle event.
    ///     Override in PlayerMobile to implement bandage interruption on war mode change.
    /// </summary>
    protected virtual void OnWarmodeToggled()
    {
    }

    /// <summary>
    ///     Sphere51a: Hook for item equip event during special actions.
    ///     Override in PlayerMobile to implement weapon re-equip during bandaging.
    /// </summary>
    /// <param name="item">The item being equipped</param>
    protected virtual void OnItemEquippedDuringAction(Item item)
    {
    }
    ...
    public virtual void Damage(int amount, Mobile from = null, bool informMount = true, bool ignoreEvilOmen = false)
    {
        ...
        if (from != null)
        {
            ...
            // Sphere51a: Hook for player-to-player damage tracking (PvP context detection)
            if (this.Player && from.Player)
            {
                from.OnDamageDealtToPlayer(this, amount);
            }
        }
    ...
    {
        // Sphere51a: War mode toggle interrupts spell casting (players only)
        if (Player)
        {
            ...
            // Sphere51a: Hook for war mode toggle (bandage interruption, etc.)
            OnWarmodeToggled();
        }
    }
...
#nullable restore
    ...
    public virtual bool EquipItem(Item item)
    {
        ...
        if (CheckEquip(item) && OnEquip(item) && item.OnEquip(this))
        {
            ...
            // Sphere51a: Hook for item equip during special actions (bandaging, etc.)
            if (Player)
            {
                OnItemEquippedDuringAction(item);
            }
    ...
    public int GetStatOffset(StatType type)
    {
        ...
        // Cap magical stat modifications at ±10
        // This prevents Bless/Curse from stacking with individual buffs/curses beyond the intended limit
        return Math.Clamp(offset, -10, 10);
    }
